...
**Stale-named secret still in active use.** The Vault entry `observing-planner-umami-secrets` (resolved via `oci-secret-store`) is intentionally not renamed despite its name — see the fuller note and prohibition in `README.md` at Step 6.4.
