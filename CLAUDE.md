...
**Retired secret, scheduled for deletion.** The Vault entry `observing-planner-umami-secrets` was retired as part of `uprinter/cicd` feature 002-umami-debrand and is no longer consumed by anything in the cluster. It is in OCI Vault's `PENDING_DELETION` state, scheduled for deletion 2026-08-24. The live equivalent is `umami-secrets` (same Vault, same structure), referenced from `uprinter/cicd`'s `apps/umami/secret.yaml`. See `README.md` at Step 6.4 for detail.
